...
const fetchWalletDetails = async () => {
  ...
  return [
    { name: 'Deposit Wallet', balance: Number(data.depositWallet) || 0 },
    { name: 'My Wallet', balance: Number(data.myWallet) || 0 },
    { name: 'URWA Wallet', balance: Number(data.emgtWallet) || 0 },
    { name: 'Principal Wallet', balance: Number(data.principalWallet) || 0 },
    { name: 'Referral Wallet', balance: Number(data.referralWallet) || 0 },
  ];
};
...
const Wallets = () => {
  ...
  // ✅ SkeletonCard defined at component level
  const SkeletonCard = () => (
    <div className="rounded-xl border border-gradient theme-card-style p-3 space-y-2 animate-pulse">
      <div className="flex flex-col-reverse gap-3 justify-between">
        <h2 className="text-sm font-semibold flex items-center gap-2">
          ...
          <span className="h-4 w-28 bg-gray-200 rounded"></span>
        </h2>
  ...
  if (isError && !isDemoMode) {
    return <p className="text-red-600 p-4">Error loading wallets: {error?.message}</p>;
  }
  ...
  return (
    <>
      {displayWallets?.map((wallet, idx) => (
        <div
          key={idx}
          className="rounded-xl border border-gradient theme-card-style p-4 space-y-3 hover:shadow-md transition-all"
        >
            ...
            {wallet.name === 'URWA Wallet' ? (
              <>
                ...
                <span className="text-sm text-gray-500 ml-2">
                  ($
                  {(Number(wallet.balance) * 0.3).toLocaleString('en-IN', {
                    minimumFractionDigits: 2,
                  })}
                  )
                </span>
...
//     { name: 'URWA Wallet', balance: emgtWallet },
