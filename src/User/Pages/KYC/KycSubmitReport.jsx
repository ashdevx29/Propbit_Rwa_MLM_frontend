...
const KycSubmitReport = () => {
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('kycId', {
        header: 'KYC ID',
        cell: (info) => (
          <span className="tracking-[0.15em]">{info.getValue() || '—'}</span>
        ),
      }),
  ...
  return (
    <div className="theme-card-style border-gradient text-gray-800 p-6 rounded-md max-w-full mx-auto">
      <div className="flex justify-between mb-6 gap-4 flex-wrap-reverse">
        <h2 className="text-2xl text-[#0F172A] droxen-font ">KYC Submission History</h2>
        <button
      ...
      <form onSubmit={handleApplyFilters} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8 items-end">
        <div>
          <label className="text-sm font-medium text-[#0082ED] mb-1 block">Search (Name / ID)</label>
          <input
            type="text"
            value={tempSearch}
        ...
        <>
          <div className="overflow-x-auto rounded-2xl border border-gray-200">
            <table className="w-full text-sm">
              <thead className="bg-[linear-gradient(135deg,#0082ED_0%,#00E8A6_100%)]/25 gued-font text-left px-6 py-4 border-b border-gray-200  text-[#0082ED] whitespace-nowrap ">
                {table.getHeaderGroups().map((headerGroup) => (
                  <tr key={headerGroup.id}>
                    {headerGroup.headers.map((header) => (
                      <th
                        key={header.id}
                        className="text-left px-6 py-4 border-b border-gray-200 font-bold text-[#0082ED]"
                      >
