...
const KycReport = () => {
  ...
  return (
    <div className="p-4 max-w-[1260px] mx-auto">
      <h2 className="text-2xl font-bold mb-6 droxen-font">KYC Management</h2>
      ...
      <div className="mb-6 bg-white p-4 rounded-2xl border shadow-sm">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          <div>
            ...
            <input type="text" value={userIdInput} onChange={e => setUserIdInput(e.target.value)} placeholder="GGL00001" className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm" />
          </div>
          <div>
          ...
          <div className="flex items-end">
            <button onClick={handleSearch} className="w-full bg-[linear-gradient(135deg,#0082ED_0%,#00E8A6_100%)] hover:bg-[#0e2a3d] text-[#000] py-3 rounded-xl text-sm font-medium">Search</button>
          </div>
        </div>
      </div>
      ...
      <div className="relative overflow-x-auto bg-white rounded-3xl shadow border">
        {isFetching && (
          <div className="absolute inset-0 bg-white/80 flex items-center justify-center z-10 rounded-3xl">
            <div className="flex items-center gap-3 text-[#0082ED] font-medium">
              <div className="w-6 h-6 border-4 border-[#4ADD97] border-t-transparent rounded-full animate-spin" />
              Loading...
            </div>
          </div>
        )}
      ...
      <div className="mt-6 flex md:flex-row flex-col gap-4 items-center justify-between text-sm">
        <div className="text-gray-600">
          ...
          of {apiResponse.totalRecords} entries
        </div>
        ...
        <div className="space-x-2 flex">
          ...
          <button
            ...
            disabled={pagination.pageIndex + 1 >= (apiResponse.totalPages || 1)}
            className="px-3 py-1 border text-xs rounded disabled:opacity-40"
          >
            Next
          </button>
          ...
          <button
            onClick={() => table.setPageIndex((apiResponse.totalPages || 1) - 1)}
            disabled={pagination.pageIndex + 1 >= (apiResponse.totalPages || 1)}
            className="px-3 py-1 border text-xs rounded disabled:opacity-40"
          >
            Last
            ...
            <div className="p-6 md:p-8 overflow-y-auto flex-1">
              ...
              <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6 mb-10">
                <div>
                  ...
                  <p className="text-3xl font-semibold text-[#0082ED]">{selectedKyc.user?.user_id}</p>
                </div>
