...
const Login = () => {
  const [form, setForm] = useState({ email: "", password: "", otp: "" , role: ""});
...
// add akshay
  ...
  const handleRequestOtp = () => {
    ...
    setErrors({});
    requestOtp({ email: form.email,role: form.role, purpose: "login" });
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2 className="text-[28px] text-center font-bold text-[#0D0D0D] mb-2">
          Welcome to Admin Panel
        </h2>
        <p className="text-[16px] text-center text-[#4AD988] mb-6">
          {otpRequested
        ...
        <form onSubmit={handleSubmit} noValidate>
          <label className="block text-[#0D0D0D] font-semibold mb-1 text-[16px]">
            Email
          </label>
          <input
            type="email"
            ...
            className={`w-full p-2 mb-1 border rounded outline-none transition duration-300 focus:ring-2 ${
              errors.email
                ...
                : "border-[#4ADD97] focus:border-[#0e9d52] focus:ring-[#0e9d52]/30"
            }`}
...
<div className="relative">
  <input
    ...
    value={form.password}
    onChange={(e) => setForm({ ...form, password: e.target.value })}
    className={`w-full p-2 pr-12 mb-1 border rounded outline-none transition duration-300 focus:ring-2 ${
      errors.password
        ? "border-red-600 focus:ring-red-400"
        : "border-[#4ADD97] focus:border-[#0e9d52] focus:ring-[#0e9d52]/30"
    }`}
...
{errors.role && (
  <p className="text-red-600 text-sm mb-3">{errors.role}</p>
)}


          {otpRequested && (
            <>
              <label className="block text-[#000] font-semibold mb-1 text-[16px]">
                OTP
              </label>
              <input
                type="text"
          ...
          <div className="flex justify-center mt-4">
            <button
              type="submit"
              disabled={isLoggingIn || isRequestingOtp}
              className="w-[120px] bg-[linear-gradient(135deg,#0082ED_0%,#00E8A6_100%)] text-white hover:opacity-90  py-2 rounded shadow-md transition duration-300 transform disabled:opacity-50"
            >
